...
async def _create_event_and_schedule(db_session, *, slots: int=3):
    event = Event(title='Тестовое мероприятие', category=EventCategory.excursion, base_price=Decimal('1000.00'), status=EventStatus.active)
    db_session.add(event)
    await db_session.flush()
    start = datetime.now(timezone.utc) + timedelta(days=1)
    schedule = Schedule(event_id=event.id, start_datetime=start, end_datetime=start + timedelta(hours=2), available_slots=slots, status=ScheduleStatus.open)
...
async def test_booking_reserves_slots_and_uses_payment_mock(client, db_session, monkeypatch):
    user_1 = await _create_user(db_session, login='booker1', email='booker1@example.com')
    user_2 = await _create_user(db_session, login='booker2', email='booker2@example.com')
    ...
    payload = {'schedule_id': schedule.id, 'participants_count': 2, 'participants': [{'first_name': 'Ivan', 'last_name': 'Ivanov', 'is_child': True}, {'first_name': 'Petr', 'last_name': 'Petrov', 'is_child': True}]}
    first = await client.post('/api/v1/bookings', json=payload, headers=_auth_headers(token_1))
    ...
    second = await client.post('/api/v1/bookings', json=payload, headers=_auth_headers(token_2))
