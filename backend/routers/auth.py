...
async def _fetch_vk_profile(code: str) -> tuple[int, str | None, str | None, str | None]:
    if not settings.VK_CLIENT_ID or not settings.VK_CLIENT_SECRET or (not settings.VK_REDIRECT_URI):
        raise HTTPException(status_code=500, detail='VK OAuth не настроен на сервере')
    async with httpx.AsyncClient(timeout=15.0) as client:
        token_resp = await client.get(VK_OAUTH_ACCESS_TOKEN_URL, params={'client_id': settings.VK_CLIENT_ID, 'client_secret': settings.VK_CLIENT_SECRET, 'redirect_uri': settings.VK_REDIRECT_URI, 'code': code})
        ...
        profile_resp = await client.get(VK_API_USERS_GET_URL, params={'access_token': access_token, 'v': VK_API_VERSION, 'fields': 'photo_200'})
...
async def _upsert_user_by_vk(db: AsyncSession, *, vk_user_id: int, email: str | None, first_name: str | None, last_name: str | None) -> User:
    existing_by_vk = (await db.execute(select(User).where(User.vk_user_id == vk_user_id))).scalar_one_or_none()
    ...
    if existing_by_email is not None:
        if existing_by_email.vk_user_id and existing_by_email.vk_user_id != vk_user_id:
            raise HTTPException(status_code=409, detail='Этот email уже привязан к другому VK-аккаунту')
    ...
    suffix = 1
    while (await db.execute(select(User.id).where(User.login == login_candidate))).scalar_one_or_none() is not None:
        suffix += 1
        login_candidate = f'{base_login}_{suffix}'
    user = User(login=login_candidate, email=safe_email, password_hash=hash_password(secrets.token_urlsafe(32)), role=UserRole.client, first_name=first_name, last_name=last_name, oauth_provider='vk', oauth_provider_user_id=str(vk_user_id), vk_user_id=vk_user_id, last_login_at=datetime.now(timezone.utc))
    db.add(user)
    await db.commit()
    await db.refresh(user)
    return user

@router.get('/vk/login-url', response_model=VkLoginUrlResponse, summary='Ссылка для входа через VK')
...
@router.post('/register', response_model=UserResponse, status_code=status.HTTP_201_CREATED, summary='Регистрация пользователя', description='Создаёт нового пользователя с ролью клиента. Логин и email должны быть уникальными.')
async def register(data: UserCreate, db: Annotated[AsyncSession, Depends(get_db)]) -> User:
    dup = await db.execute(select(User.id).where(or_(User.login == data.login, User.email == str(data.email))))
    ...
    user = User(login=data.login, email=str(data.email), password_hash=hash_password(data.password), role=UserRole.client, first_name=data.first_name, last_name=data.last_name, patronymic=data.patronymic, phone=data.phone)
    db.add(user)
    await db.commit()
    await db.refresh(user)
    return user

@router.post('/login', response_model=Token, summary='Вход в систему', description='Проверяет логин/email и пароль, затем возвращает JWT токен доступа.')
async def login(body: LoginRequest, db: Annotated[AsyncSession, Depends(get_db)]) -> Token:
    result = await db.execute(select(User).where(or_(User.login == body.login, User.email == body.login)))
...
@router.get('/me', response_model=UserResponse, summary='Профиль текущего пользователя', description='Возвращает данные пользователя по Bearer-токену.')
async def read_me(current_user: Annotated[User, Depends(get_current_user)]) -> User:
    return current_user

@router.put('/me', response_model=UserResponse, summary='Обновление профиля', description='Изменяет данные текущего пользователя. Можно обновить логин, email, пароль и персональные поля.')
async def update_me(body: UserUpdate, current_user: Annotated[User, Depends(get_current_user)], db: Annotated[AsyncSession, Depends(get_db)]) -> User:
    if body.login is not None:
        taken = await db.execute(select(User.id).where(User.login == body.login, User.id != current_user.id))
    ...
    if body.email is not None:
        ...
        taken = await db.execute(select(User.id).where(User.email == email_str, User.id != current_user.id))
        if taken.scalar_one_or_none() is not None:
