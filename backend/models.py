...
class User(Base):
    ...
    role: Mapped[UserRole] = mapped_column(Enum(UserRole, name='user_role', native_enum=True), default=UserRole.client)
    first_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    last_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    patronymic: Mapped[str | None] = mapped_column(String(255), nullable=True)
    phone: Mapped[str | None] = mapped_column(String(32), nullable=True)
    avatar_url: Mapped[str | None] = mapped_column(String(1024), nullable=True)
    guide_id: Mapped[int | None] = mapped_column(ForeignKey('guides.id', ondelete='SET NULL'), nullable=True, unique=True)
    ...
    guide_profile: Mapped[Guide | None] = relationship(back_populates='user_account', foreign_keys=[guide_id])
...
class Guide(Base):
    ...
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    availability_status: Mapped[GuideAvailabilityStatus] = mapped_column(Enum(GuideAvailabilityStatus, name='guide_availability_status', native_enum=False, validate_strings=True), default=GuideAvailabilityStatus.active)
    schedules: Mapped[list[Schedule]] = relationship(back_populates='guide', foreign_keys='Schedule.guide_id')
    user_account: Mapped[User | None] = relationship(back_populates='guide_profile', foreign_keys=[User.guide_id])
    salary_events: Mapped[list[GuideSalaryEvent]] = relationship(back_populates='guide', cascade='all, delete-orphan')
    chat_messages: Mapped[list[GuideChatMessage]] = relationship(back_populates='guide', cascade='all, delete-orphan')
...
class Event(Base):
    ...
    category: Mapped[EventCategory] = mapped_column(Enum(EventCategory, name='event_category', native_enum=True))
    ...
    status: Mapped[EventStatus] = mapped_column(Enum(EventStatus, name='event_status', native_enum=True), default=EventStatus.active)
...
class Schedule(Base):
    ...
    status: Mapped[ScheduleStatus] = mapped_column(Enum(ScheduleStatus, name='schedule_status', native_enum=True), default=ScheduleStatus.open)
    guide_id: Mapped[int | None] = mapped_column(ForeignKey('guides.id', ondelete='SET NULL'), nullable=True)
    rejected_by_guide_id: Mapped[int | None] = mapped_column(ForeignKey('guides.id', ondelete='SET NULL'), nullable=True)
    guide_confirmed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    guide_rejected_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    guide_reject_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    guide_completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    event: Mapped[Event] = relationship(back_populates='schedules')
    guide: Mapped[Guide | None] = relationship(back_populates='schedules', foreign_keys=lambda: [Schedule.guide_id])
    bookings: Mapped[list[Booking]] = relationship(back_populates='schedule', cascade='all, delete-orphan')
    salary_event: Mapped[GuideSalaryEvent | None] = relationship(back_populates='schedule', uselist=False)
...
class Booking(Base):
    ...
    status: Mapped[BookingStatus] = mapped_column(Enum(BookingStatus, name='booking_status', native_enum=True), default=BookingStatus.pending)
    ...
    transactions: Mapped[list[Transaction]] = relationship(back_populates='booking', cascade='all, delete-orphan')
...
class Transaction(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    booking_id: Mapped[int] = mapped_column(ForeignKey('bookings.id', ondelete='CASCADE'))
    payment_method: Mapped[PaymentMethod] = mapped_column(Enum(PaymentMethod, name='payment_method', native_enum=True))
    amount: Mapped[Decimal] = mapped_column(Numeric(12, 2))
    status: Mapped[TransactionStatus] = mapped_column(Enum(TransactionStatus, name='transaction_status', native_enum=True), default=TransactionStatus.pending)
...
class Settings(Base):
    ...
    data_type: Mapped[SettingDataType] = mapped_column(Enum(SettingDataType, name='setting_data_type', native_enum=True), default=SettingDataType.string)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class GuideSalaryEvent(Base):
    ...
    schedule_id: Mapped[int] = mapped_column(ForeignKey('schedules.id', ondelete='CASCADE'), unique=True)
