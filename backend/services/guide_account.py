...
async def ensure_guide_profile(db: AsyncSession, user: User) -> User:
    ...
    if match_filters:
        existing = (await db.execute(select(Guide).where(or_(*match_filters)).order_by(Guide.id.asc()))).scalars().first()
        if existing is not None:
            occupied = (await db.execute(select(User.id).where(User.guide_id == existing.id, User.id != user.id))).scalar_one_or_none()
            if occupied is None:
    ...
    if not last:
        last = 'Гид'
    guide = Guide(first_name=first, last_name=last, patronymic=user.patronymic, phone=user.phone, email=user.email, is_active=True, availability_status=GuideAvailabilityStatus.active)
    db.add(guide)
    await db.flush()
